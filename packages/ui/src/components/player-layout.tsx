...
function PlayerWorkspace({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn(
				"grid gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px]",
				className,
			)}
			{...props}
		/>
...
function PlayerAside({ className, ...props }: React.ComponentProps<"aside">) {
	return (
		<aside
			...
			className={cn(
				"hidden min-h-0 rounded-lg border border-border bg-card lg:fixed lg:top-[4.25rem] lg:right-3 lg:z-10 lg:flex lg:max-h-[calc(100dvh-4.25rem)] lg:w-80 lg:flex-col xl:w-[360px]",
				className,
			)}
			{...props}
		/>
...
function PlayerTabletStack({
	...
	return (
		<section
			...
			className={cn("hidden gap-3 md:grid lg:hidden", className)}
			{...props}
		/>
	);
}
